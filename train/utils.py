...
def get_cifar_gen():
 ...
 #   x_train=np.load('PIE27_x.npy')
#    x_train=x_train.reshape(3329,32,32,1)
 #   y_train=np.load('PIE27_y.npy')
  #  y_train -=1
   # y_train = tf.keras.utils.to_categorical(y_train)
    #x_test=np.load('PIE05_x.npy')
#    x_test=x_test.reshape(3332,32,32,1)
 #   y_test=np.load('PIE05_y.npy')
  #  y_test-=1
   # y_test = tf.keras.utils.to_categorical(y_test)
    #testgen = ImageDataGenerator()
#    cifar_gen=testgen.flow(x_train,y_train,batch_size=batch_size)
